...
2. Track the live crypto price versus the market's opening/reference price.
...
Polymarket states these markets resolve from Chainlink BTC/USD or ETH/USD data streams. The default price client uses CoinGecko as a public paper-trading proxy because it is easy to access without credentials. Before live trading, replace `CoinGeckoPriceClient` with the exact resolution feed or a low-latency professional feed aligned to Chainlink.
